...
// *do not* try to interpret the `stats_tf_...` select‑menu id here because that
...
// previous special‑case that incorrectly mapped "tf" back to "networth".
...
function renderOverview(username, wallet) {
  ...
  return {
    ...
    fields: [
      ...
      {
        name: '▸ Income / Spending Breakdown',
        value: breakdownText,
        inline: false,
      },
    ],
...
function renderXpPage(username, userId, wallet) {
  ...
  const filled = xp.xpToNext > 0
    ...
    : BAR_TOTAL;
  const bar = '◆'.repeat(Math.min(filled, BAR_TOTAL)) + '◇'.repeat(Math.max(0, BAR_TOTAL - filled));

  // 10-parallelogram level bar (one pip per 10 levels)
  const levelPips = Math.floor(xp.level / 10);
  const levelBar = '◆'.repeat(levelPips) + '◇'.repeat(Math.max(0, 10 - levelPips));
  ...
  if (xpRank !== null && xpRank !== undefined) {
    baseEmbed.fields.push({ name: '🏅 XP Rank', value: `#${xpRank}`, inline: false });
  }
  return baseEmbed;
}
...
async function renderXpChartPage(username, userId, wallet, timeframeKey = STATS_DEFAULT_TIMEFRAME_KEY) {
  try {
    ...
    return { content: '', embeds: [{ title: `⭐ ${username}'s XP`, color: 0x2b2d31, description: 'Error rendering XP data.' }] };
  }
}
...
function renderFinancePage(username, userId, wallet) {
  ...
  return {
    title: `💰 ${username}'s Finance`,
    color: 0x2b2d31,
    description: '> Breakdown of earnings and losses by activity',
    fields: [
      { name: '▸ Income / Spending', value: breakdownText, inline: false },
    ],
  };
}
...
async function renderCollectiblesPage(username, userId, wallet, timeframeKey = STATS_DEFAULT_TIMEFRAME_KEY) {
  try {
    ...
    if (collectRank !== null && collectRank !== undefined) {
      collectEmbed.fields.push({ name: '🏅 Collection Rank', value: `#${collectRank}`, inline: false });
    }
    ...
    return { content: '', embeds: [{ title: `📦 ${username}'s Collection`, color: 0x2b2d31, description: 'Error rendering collection data.' }] };
  }
}
